...
Aggregate  (cost=16465.62..16465.63 rows=1 width=64)
  ->  Nested Loop  (cost=7.64..16465.61 rows=1 width=22)
        Join Filter: (mi_idx.movie_id = t.id)
        ->  Nested Loop  (cost=7.21..16465.11 rows=1 width=13)
              Join Filter: (mi_idx.info_type_id = it.id)
              ->  Seq Scan on info_type it  (cost=0.00..2.41 rows=1 width=4)
                    Filter: ((info)::text = 'rating'::text)
              ->  Nested Loop  (cost=7.21..16462.59 rows=9 width=17)
