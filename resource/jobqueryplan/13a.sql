...
Finalize Aggregate  (cost=20127.12..20127.13 rows=1 width=96)
  ->  Gather  (cost=20126.89..20127.10 rows=2 width=96)
        Workers Planned: 2
        ->  Partial Aggregate  (cost=19126.89..19126.90 rows=1 width=96)
              ->  Hash Join  (cost=8.73..19126.81 rows=11 width=65)
                    Hash Cond: (mi.info_type_id = it1.id)
                    ->  Nested Loop  (cost=6.30..19120.98 rows=1250 width=69)
                          ->  Nested Loop  (cost=5.87..19051.21 rows=36 width=34)
                          ...
                          ->  Index Scan using movie_id_movie_info on movie_info mi  (cost=0.43..1.51 rows=43 width=51)
                                Index Cond: (movie_id = t.id)
                    ->  Hash  (cost=2.41..2.41 rows=1 width=4)
