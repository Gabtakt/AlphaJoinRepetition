...
Aggregate  (cost=4164.60..4164.61 rows=1 width=64)
  ->  Nested Loop  (cost=9.48..4160.45 rows=830 width=33)
        Join Filter: (n.id = an.person_id)
        ->  Nested Loop  (cost=9.06..3986.76 rows=357 width=25)
              ->  Nested Loop  (cost=8.63..3824.36 rows=357 width=21)
                    Join Filter: (t.id = ci.movie_id)
                    ->  Nested Loop  (cost=8.06..3790.08 rows=17 width=29)
                          ->  Nested Loop  (cost=7.64..3769.14 rows=47 width=33)
                                Join Filter: (t.id = mc.movie_id)
                                ->  Nested Loop  (cost=7.21..3763.72 rows=9 width=25)
