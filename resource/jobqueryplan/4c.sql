...
Aggregate  (cost=16484.33..16484.34 rows=1 width=64)
  ->  Nested Loop  (cost=7.64..16484.30 rows=6 width=22)
        Join Filter: (mi_idx.movie_id = t.id)
        ->  Nested Loop  (cost=7.21..16480.29 rows=8 width=13)
              Join Filter: (mi_idx.info_type_id = it.id)
              ->  Seq Scan on info_type it  (cost=0.00..2.41 rows=1 width=4)
                    Filter: ((info)::text = 'rating'::text)
              ->  Nested Loop  (cost=7.21..16466.97 rows=873 width=17)
                    ...
                    ->  Index Scan using movie_id_movie_info_idx on movie_info_idx mi_idx  (cost=0.43..0.50 rows=2 width=13)
