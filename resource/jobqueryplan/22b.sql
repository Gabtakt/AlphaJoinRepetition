...
Aggregate  (cost=7442.14..7442.15 rows=1 width=96)
  ->  Nested Loop  (cost=8.92..7442.14 rows=1 width=41)
        Join Filter: (mi.info_type_id = it1.id)
        ->  Nested Loop  (cost=8.92..7439.71 rows=1 width=45)
              Join Filter: (t.id = mi.movie_id)
              ->  Nested Loop  (cost=8.49..7438.10 rows=1 width=57)
                    Join Filter: (mc.company_type_id = ct.id)
                    ->  Nested Loop  (cost=8.49..7437.01 rows=1 width=61)
                          ->  Nested Loop  (cost=8.07..7436.47 rows=1 width=46)
                                Join Filter: (t.id = mc.movie_id)
                                ->  Nested Loop  (cost=7.64..7435.89 rows=1 width=34)
                                      Join Filter: (mi_idx.info_type_id = it.id)
                                      ->  Seq Scan on info_type it  (cost=0.00..2.41 rows=1 width=4)
                                            Filter: ((info)::text = 'rating'::text)
                                      ->  Nested Loop  (cost=7.64..7433.20 rows=22 width=38)
                                            Join Filter: (t.id = mi_idx.movie_id)
                                            ->  Nested Loop  (cost=7.21..7428.92 rows=8 width=25)
                                                  Join Filter: (t.kind_id = kt.id)
                                                  ->  Nested Loop  (cost=7.21..7427.03 rows=28 width=29)
