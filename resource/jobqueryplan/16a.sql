...
Aggregate  (cost=3802.53..3802.54 rows=1 width=64)
  ->  Nested Loop  (cost=9.48..3802.14 rows=78 width=33)
        Join Filter: (n.id = an.person_id)
        ->  Nested Loop  (cost=9.06..3785.60 rows=34 width=25)
              ->  Nested Loop  (cost=8.63..3770.13 rows=34 width=21)
                    Join Filter: (t.id = ci.movie_id)
                    ->  Nested Loop  (cost=8.06..3766.10 rows=2 width=29)
                          ->  Nested Loop  (cost=7.64..3764.32 rows=4 width=33)
                                Join Filter: (t.id = mc.movie_id)
                                ->  Nested Loop  (cost=7.21..3763.72 rows=1 width=25)
