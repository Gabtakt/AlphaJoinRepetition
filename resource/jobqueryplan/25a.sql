...
Aggregate  (cost=7521.31..7521.32 rows=1 width=128)
  ->  Nested Loop  (cost=1012.06..7521.30 rows=1 width=80)
        Join Filter: (mi.movie_id = t.id)
        ->  Nested Loop  (cost=1011.63..7520.48 rows=1 width=79)
              ->  Nested Loop  (cost=1011.20..7519.45 rows=1 width=68)
                    ->  Nested Loop  (cost=1011.05..7519.27 rows=1 width=72)
                          Join Filter: (mi.movie_id = ci.movie_id)
                          ->  Gather  (cost=1010.49..7489.58 rows=1 width=64)
                                Workers Planned: 2
                                ->  Nested Loop  (cost=10.49..6489.48 rows=1 width=64)
                                      Join Filter: (mi_idx.movie_id = mi.movie_id)
                                      ->  Hash Join  (cost=10.05..6453.02 rows=2 width=13)
                                            Hash Cond: (mi_idx.info_type_id = it.id)
                                            ->  Nested Loop  (cost=7.63..6449.98 rows=226 width=17)
